...
class BasketTableCell: UITableViewCell {
    ...
    func setAmount(){
        if(self.appDelegate.lang_id == 2){
            self.lblPrice.text = "\(AppFunc().decimalNum(self.item.priceVND*self.item.amount)) \(appDelegate.currency)"
        } else {
            self.lblPrice.text = "\(AppFunc().decimalNum(self.item.price*self.item.amount)) \(appDelegate.currency)"
        }
